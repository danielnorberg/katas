...
public class LongBitSet implements Cloneable, Serializable {
  ...
  public long nextSetBit(long fromIndex) {
    ...
    while (true) {
      if (word != 0) {
        return ((long) u * (long) BITS_PER_WORD) + (long) Long.numberOfTrailingZeros(word);
      }
      if (++u == wordsInUse) {
        return -1;
      }
  ...
  public long nextClearBit(long fromIndex) {
    ...
    while (true) {
      ...
      if (++u == wordsInUse) {
        return (long) wordsInUse * (long) BITS_PER_WORD;
      }
      word = ~words[u];
    }
  }
  ...
  public long previousSetBit(long fromIndex) {
    ...
    while (true) {
      if (word != 0) {
        return ((long) u + 1) * (long) BITS_PER_WORD - 1L - (long) Long.numberOfLeadingZeros(word);
      }
      if (u-- == 0) {
        return -1;
      }
  ...
  public long length() {
    ...
    return (long) BITS_PER_WORD * (long) (wordsInUse - 1) +
           (long) (BITS_PER_WORD - Long.numberOfLeadingZeros(words[wordsInUse - 1]));
  }
