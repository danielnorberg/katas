...
public class Main {
  ...
  public static void main(final String... args) {
    assertThat(SegmentTree.of(1).min(), is(1));
    ...
    assertThat(SegmentTree.of(3, 1, 4, 2).min(2, 2), is(4));
    assertThat(SegmentTree.of(3, 1, 4, 2).min(2, 3), is(2));
    ...
    for (int i = 0; i < n; i++) {
      for (int j = i; j < n; j++) {
        int expectedMin = IntStream.of(values).skip(i).limit(j - i + 1).min().getAsInt();
